	...
	if(typeof (s) != "undefined") {
		...
		for(var i = 0; i < sArr.length; i++){
			if(uSinaEmotionsHt.containsKey(sArr[i])) {
				var reStr = `<img src="${uSinaEmotionsHt.get(sArr[i])}" style="width: 24px;height: 24px;">`;
...
(function($){
	$.fn.SinaEmotion = function(target){
		...
		$(this).click(function(event){
			...
			var eTop = target.offset().top - 200;
		...
		function showEmotions(){
			...
			for(var i = page * 72; i < (page + 1) * 72 && i < emotions[category].length; ++i){
				$('#emotions .container').append($('<a href="javascript:void(0);" title="' + emotions[category][i].name + '"><img src="' + emotions[category][i].icon + '" alt="' + emotions[category][i].name + '" width="20" height="20" /></a>'));
			}
